...
import predictor_location as ploc
...
@st.cache_resource(show_spinner=False)
def load_all_location_models():
    return ploc.load_all_models_location()


def humanize_loc(label: str) -> str:
    return label.replace("equip_", "").replace("_", " ").strip().capitalize()


LOCALISATIONS_LOCATION = sorted({
    "Palmeraie", "Massira", "Guéliz", "Hivernage", "Targa", "Semlalia", "Ancienne Médina",
    "Centre Ville", "Majorelle", "Route de Casablanca", "Route d'Amezmiz", "Sidi Youssef Ben Ali",
    "Daoudiate", "Issil", "Agdal", "M'Hamid", "Autre",
})


def render_location_estimator() -> None:
    """Onglet d'estimation des LOYERS (location), basé sur les modèles
    chargés depuis models_location/, avec formulaires dédiés par
    catégorie (schéma différent des modèles de vente)."""
    try:
        loaded = load_all_location_models()
    except FileNotFoundError as e:
        st.error(
            "Dossier `models_location/` introuvable ou incomplet. Assurez-vous que les "
            f"6 fichiers `.pkl` de location sont bien présents dans le repo déployé.\n\n{e}"
        )
        st.stop()

    categorie_labels_loc = {
        "villas": "Villas", "maisons": "Maisons", "bureaux": "Bureaux",
        "appartements": "Appartements", "magasins": "Magasins", "riads": "Riads",
    }

    categorie_choice = st.selectbox(
        "Type de bien",
        options=list(categorie_labels_loc.keys()),
        format_func=lambda c: categorie_labels_loc[c],
        key="loc_categorie",
    )
    ...
    equip_list = ploc.get_equip_features_location(categorie_choice)
    prix = None
    avertissement = None

    if categorie_choice == "villas":
        c1, c2 = st.columns(2)
        with c1:
            surface_totale = st.number_input("Surface totale (m²)", min_value=1.0, value=400.0, key="loc_v_st")
            chambres = st.number_input("Chambres", min_value=0, value=4, key="loc_v_ch")
            nb_etages = st.number_input("Nombre d'étages", min_value=0, value=2, key="loc_v_et")
            caution = st.selectbox("Caution", ["Aucune", "1 mois", "2 mois", "3 mois"], index=2, key="loc_v_ca")
        with c2:
            surface_habitable = st.number_input("Surface habitable (m²)", min_value=1.0, value=300.0, key="loc_v_sh")
            salle_de_bain = st.number_input("Salles de bain", min_value=0, value=3, key="loc_v_sdb")
            salons = st.number_input("Salons", min_value=0, value=2, key="loc_v_sal")
            localisation = st.selectbox("Localisation", LOCALISATIONS_LOCATION, key="loc_v_loc")
        eq = _render_equip_checkboxes(equip_list, "loc_v")
        use_model = st.radio("Modèle", ["lgb", "xgb"], horizontal=True, key="loc_v_model")
        if st.button("Estimer le loyer", type="primary", width="stretch", key="loc_v_btn"):
            with st.spinner("Calcul de l'estimation..."):
                prix = ploc.predict_villa(
                    surface_totale, surface_habitable, chambres, salle_de_bain,
                    nb_etages, salons, caution, localisation, eq, use_model,
                )

    elif categorie_choice == "maisons":
        c1, c2 = st.columns(2)
        with c1:
            surface_habitable = st.number_input("Surface habitable (m²)", min_value=1.0, value=180.0, key="loc_m_sh")
            chambres = st.number_input("Chambres", min_value=0, value=3, key="loc_m_ch")
            etage = st.number_input("Étage", min_value=0, value=1, key="loc_m_et")
        with c2:
            surface_terrain = st.number_input("Surface terrain (m²)", min_value=0.0, value=250.0, key="loc_m_str")
            salles_bain = st.number_input("Salles de bain", min_value=0, value=2, key="loc_m_sdb")
            salons = st.number_input("Salons", min_value=0, value=2, key="loc_m_sal")
        localisation = st.selectbox("Localisation", LOCALISATIONS_LOCATION, key="loc_m_loc")
        eq = _render_equip_checkboxes(equip_list, "loc_m")
        if st.button("Estimer le loyer", type="primary", width="stretch", key="loc_m_btn"):
            with st.spinner("Calcul de l'estimation..."):
                prix = ploc.predict_maison(
                    surface_habitable, surface_terrain, chambres, salles_bain,
                    etage, salons, localisation, eq,
                )
                avertissement = ploc.AVERTISSEMENTS_LOCATION["maisons"]

    elif categorie_choice == "bureaux":
        c1, c2 = st.columns(2)
        with c1:
            surface_m2 = st.number_input("Surface (m²)", min_value=1.0, value=120.0, key="loc_b_sm2")
            nb_pieces = st.number_input("Nombre de pièces", min_value=0, value=4, key="loc_b_np")
            salle_de_bain = st.number_input("Salles de bain", min_value=0, value=1, key="loc_b_sdb")
        with c2:
            nb_etage = st.number_input("Étage", min_value=0, value=2, key="loc_b_et")
            chambres = st.number_input("Chambres", min_value=0, value=0, key="loc_b_ch")
            frais_syndic = st.number_input("Frais syndic (DH)", min_value=0.0, value=300.0, key="loc_b_fs")
        localisation = st.selectbox("Localisation", LOCALISATIONS_LOCATION, key="loc_b_loc")
        eq = _render_equip_checkboxes(equip_list, "loc_b")
        use_model = st.radio("Modèle", ["lgb", "xgb"], horizontal=True, key="loc_b_model")
        if st.button("Estimer le loyer", type="primary", width="stretch", key="loc_b_btn"):
            with st.spinner("Calcul de l'estimation..."):
                prix = ploc.predict_bureau(
                    surface_m2, nb_etage, nb_pieces, chambres, salle_de_bain,
                    frais_syndic, localisation, eq, use_model,
                )

    elif categorie_choice == "appartements":
        c1, c2 = st.columns(2)
        with c1:
            surface_totale = st.number_input("Surface totale (m²)", min_value=1.0, value=90.0, key="loc_a_st")
            chambres = st.number_input("Chambres", min_value=0, value=2, key="loc_a_ch")
            nb_etages = st.number_input("Étage", min_value=0, value=3, key="loc_a_et")
            type_appartement = st.text_input("Type d'appartement", value="Appartement", key="loc_a_type")
        with c2:
            surface_habitable = st.number_input("Surface habitable (m²)", min_value=1.0, value=85.0, key="loc_a_sh")
            salle_de_bain = st.number_input("Salles de bain", min_value=0, value=1, key="loc_a_sdb")
            salons = st.number_input("Salons", min_value=0, value=1, key="loc_a_sal")
            caution = st.selectbox("Caution", ["Aucune", "1 mois", "2 mois", "3 mois"], index=2, key="loc_a_ca")
        frais_syndic = st.number_input("Frais syndic (DH)", min_value=0.0, value=200.0, key="loc_a_fs")
        localisation = st.selectbox("Localisation", LOCALISATIONS_LOCATION, key="loc_a_loc")
        eq = _render_equip_checkboxes(equip_list, "loc_a")
        use_model = st.radio("Modèle", ["lgb", "xgb"], horizontal=True, key="loc_a_model")
        if st.button("Estimer le loyer", type="primary", width="stretch", key="loc_a_btn"):
            with st.spinner("Calcul de l'estimation..."):
                prix = ploc.predict_appartement(
                    surface_totale, surface_habitable, chambres, salle_de_bain,
                    nb_etages, salons, frais_syndic, type_appartement, caution,
                    localisation, eq, use_model,
                )

    elif categorie_choice == "magasins":
        surface_m2 = st.number_input("Surface (m²)", min_value=1.0, value=80.0, key="loc_mg_sm2")
        salle_de_bain = st.number_input("Salles de bain", min_value=0, value=0, key="loc_mg_sdb")
        localisation = st.selectbox("Localisation", LOCALISATIONS_LOCATION, key="loc_mg_loc")
        eq = _render_equip_checkboxes(equip_list, "loc_mg")
        use_model = st.radio("Modèle", ["lgb", "xgb"], horizontal=True, key="loc_mg_model")
        if st.button("Estimer le loyer", type="primary", width="stretch", key="loc_mg_btn"):
            with st.spinner("Calcul de l'estimation..."):
                prix = ploc.predict_magasin(surface_m2, salle_de_bain, localisation, eq, use_model)

    elif categorie_choice == "riads":
        c1, c2 = st.columns(2)
        with c1:
            surface_m2 = st.number_input("Surface (m²)", min_value=1.0, value=250.0, key="loc_r_sm2")
            nb_etages = st.number_input("Nombre d'étages", min_value=0, value=3, key="loc_r_et")
            chambres = st.number_input("Chambres", min_value=0, value=5, key="loc_r_ch")
        with c2:
            surface_habitable = st.number_input("Surface habitable (m²)", min_value=0.0, value=220.0, key="loc_r_sh")
            salles_de_bain = st.number_input("Salles de bain", min_value=0, value=4, key="loc_r_sdb")
            salons = st.number_input("Salons", min_value=0, value=2, key="loc_r_sal")
        caution = st.selectbox("Caution", ["Aucune", "1 mois", "2 mois", "3 mois"], index=2, key="loc_r_ca")
        localisation = st.selectbox("Localisation", LOCALISATIONS_LOCATION, index=LOCALISATIONS_LOCATION.index("Ancienne Médina") if "Ancienne Médina" in LOCALISATIONS_LOCATION else 0, key="loc_r_loc")
        eq = _render_equip_checkboxes(equip_list, "loc_r")
        if st.button("Estimer le loyer", type="primary", width="stretch", key="loc_r_btn"):
            with st.spinner("Calcul de l'estimation..."):
                prix = ploc.predict_riad(
                    surface_m2, surface_habitable, nb_etages, chambres,
                    salles_de_bain, salons, caution, localisation, eq,
                )
                avertissement = ploc.AVERTISSEMENTS_LOCATION["riads"]

    if prix is not None:
        prix_fmt = f"{prix:,}".replace(",", " ")
        footnote = "Estimation basée sur les modèles de loyers entraînés sur des annonces locatives de Marrakech."
        if avertissement:
            footnote = avertissement
        st.markdown(
            f"""
            <div class="result-card">
                <div class="result-label">Loyer estimé</div>
                <div class="result-price">{prix_fmt} DH/mois</div>
                <div class="result-footnote">{footnote}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def _render_equip_checkboxes(equip_list, prefix: str, n_cols: int = 3) -> dict:
    eq = {}
    if not equip_list:
        return eq
    st.markdown('<div class="section-label">Équipements</div>', unsafe_allow_html=True)
    cols = st.columns(n_cols)
    for i, name in enumerate(equip_list):
        with cols[i % n_cols]:
            eq[name] = 1 if st.checkbox(humanize_loc(name), key=f"{prefix}_eq_{name}") else 0
    return eq
...
st.markdown(
    ...
    <div class="gateone-hero">
        ...
        <p>Estimez le prix d'achat ou le loyer mensuel d'un bien, et explorez les
        tendances du marché immobilier à Marrakech, à partir de modèles XGBoost /
        LightGBM entraînés sur des milliers d'annonces réelles.</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
@st.cache_resource(show_spinner=False)
def load_loc_dashboard_stats() -> Optional[dict]:
    path = os.path.join(MODEL_DIR, "loc_dashboard_stats.json")
    if not os.path.exists(path):
        return None
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)
...
def render_location_dashboard() -> None:
    stats = load_loc_dashboard_stats()
    if stats is None:
        st.warning("Fichier `models/loc_dashboard_stats.json` introuvable.")
        return

    CAT_LABELS = {
        "appartements": "Appartements", "villas": "Villas", "maisons": "Maisons",
        "bureaux": "Bureaux", "magasins": "Magasins", "riads": "Riads",
    }
    CAT_COLORS = {
        "appartements": "#3B4FA0", "villas": "#C1602E", "maisons": "#2E8B57",
        "bureaux": "#7B5EA7", "magasins": "#D4A017", "riads": "#C05070",
    }

    # ── KPI globaux ──────────────────────────────────────────────
    st.markdown('<div class="section-label">Vue d\'ensemble</div>', unsafe_allow_html=True)
    all_medians = {cat: int(np.median(list(q.values()))) for cat, q in stats.items()}
    kpi_cols = st.columns(len(all_medians))
    for i, (cat, med) in enumerate(all_medians.items()):
        with kpi_cols[i]:
            st.markdown(
                f"""<div class="kpi-card">
                    <div class="kpi-label">{CAT_LABELS.get(cat, cat)}</div>
                    <div class="kpi-value">{fmt_dh(med)}</div>
                    <div class="kpi-sub">DH/mois médian</div>
                </div>""",
                unsafe_allow_html=True,
            )
    ...
    st.markdown('<div class="section-label">Loyer médian par catégorie (DH/mois)</div>', unsafe_allow_html=True)
    fig_cat = go.Figure(data=[
        go.Bar(
            x=[CAT_LABELS.get(c, c) for c in all_medians],
            y=list(all_medians.values()),
            marker_color=[CAT_COLORS.get(c, "#3B4FA0") for c in all_medians],
            text=[f"{fmt_dh(v)} DH" for v in all_medians.values()],
            textposition="outside",
        )
    ])
    fig_cat.update_layout(
        ...
        font=dict(family="Inter, sans-serif", color="#1C1815", size=13),
        yaxis=dict(title="Loyer médian (DH/mois)", gridcolor="#EFE8DA"),
        showlegend=False,
    )
    st.plotly_chart(fig_cat, width="stretch", config={"displayModeBar": False})

    # ── Détail par catégorie ─────────────────────────────────────
    st.markdown('<div class="section-label">Loyers par quartier</div>', unsafe_allow_html=True)
    ...
    quartiers = stats[cat_choice]
    color = CAT_COLORS.get(cat_choice, "#3B4FA0")

    col_left, col_right = st.columns(2)

    with col_left:
        st.markdown('<div class="section-label">Quartiers les plus chers</div>', unsafe_allow_html=True)
        top10 = list(quartiers.items())[:10]
        rows = "".join(
            f'<div class="rank-row"><div><span class="rank-name">{q}</span></div>'
            f'<span class="rank-value">{fmt_dh(v)} DH</span></div>'
            for q, v in top10
        )
        st.markdown(f'<div class="kpi-card">{rows}</div>', unsafe_allow_html=True)

    with col_right:
        st.markdown('<div class="section-label">Quartiers les moins chers</div>', unsafe_allow_html=True)
        bottom10 = list(quartiers.items())[-10:][::-1]
        rows = "".join(
            f'<div class="rank-row"><div><span class="rank-name">{q}</span></div>'
            f'<span class="rank-value">{fmt_dh(v)} DH</span></div>'
            for q, v in bottom10
    ...
    # ── Graphique horizontal top 15 ──────────────────────────────
    st.markdown('<div class="section-label">Top 15 quartiers — loyer médian estimé (DH/mois)</div>', unsafe_allow_html=True)
    top15 = list(quartiers.items())[:15]
    top15_sorted = sorted(top15, key=lambda x: x[1])
    fig_q = go.Figure(data=[
        go.Bar(
            x=[v for _, v in top15_sorted],
            y=[q for q, _ in top15_sorted],
            orientation="h",
            marker_color=color,
            text=[f"{fmt_dh(v)} DH" for _, v in top15_sorted],
            textposition="outside",
        )
    ])
    fig_q.update_layout(
        ...
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        font=dict(family="Inter, sans-serif", color="#1C1815", size=13),
        xaxis=dict(title="Loyer médian estimé (DH/mois)", gridcolor="#EFE8DA"),
    )
    st.plotly_chart(fig_q, width="stretch", config={"displayModeBar": False})
    st.caption("Loyers médians estimés à partir des encodages TargetEncoder entraînés sur les annonces locatives de Marrakech.")


mode = st.radio(
    "Mode",
    options=["🏷️ Vente", "🔑 Location"],
    horizontal=True,
...
if mode == "🏷️ Vente":
    ...
    # ════════════════════════════════════════════════════════════
    # ONGLET 2 — DASHBOARD DU MARCHÉ (VENTE)
    # ════════════════════════════════════════════════════════════
    with tab_dashboard:
    ...
    # MODE LOCATION
    # ════════════════════════════════════════════════════════════
    tab_loc_est, tab_loc_dash = st.tabs(["🔑 Estimer un loyer", "📊 Dashboard du marché locatif"])

    with tab_loc_est:
        render_location_estimator()
