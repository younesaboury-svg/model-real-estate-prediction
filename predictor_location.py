...
MODEL_DIR_LOCATION = os.path.join(os.path.dirname(os.path.abspath(__file__)), "models")
...
def load_all_models_location():
    for cat in _CATEGORIES_LOCATION:
        path = os.path.join(MODEL_DIR_LOCATION, f"loc_model_{cat}.pkl")
...
def get_localisations_location(cat: str) -> list:
    """Retourne la liste triée des quartiers connus pour cette catégorie,
    extraite directement du TargetEncoder du modèle chargé."""
    art = _get_art(cat)
    te = art["target_enc"]
    col_key = [k for k in te.mapping.keys() if "localisation" in k][0]
    oe = te.ordinal_encoder
    loc_map = next(m["mapping"] for m in oe.category_mapping if m["col"] == col_key)
    locs = sorted([str(k) for k in loc_map.keys() if str(k) not in ("NaN", "nan", "-1")])
    return locs
